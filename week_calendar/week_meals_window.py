...
class FoodList(QListWidget):
    ...
    def dropEvent(self, e):
        ...
        if self.meal_cell:
            ...
            date = self.date_key
            day_data = self.parent_page.meal_data[date]

            self.parent_page.meal_data_changed.emit(date, day_data)
...
class MealCell(QFrame):
    ...
    # ALWAYS STRING
        self.parent_page.go_to_daily.emit(self.date_key)
        self.parent_page.day_selected.emit(self.date_key)
...
class MealPlannerPage(QWidget):
    ...
    def init_ui(self):
        ...
        self.mood_label.clicked.connect(self.on_emoji_clicked)
    ...
    def calendar_date_changed(self, selected_date):
        date_str = selected_date.toString("yyyy-MM-dd")
        self.jump_to_week_day.emit(date_str)
    
    def emoji_button_clicked(self, date, meal_data):
        self.meal_data[date] = meal_data
        self.meal_data_changed.emit(date, meal_data)
        self.jump_to_week_day.emit(date)

    def _on_date_picked(self,dt):
        self.current_date=dt; self.update_week()

    def select_date(self, date_str):
        """Alias to match expected API"""
        if hasattr(self, "select_day"):
            self.select_day(date_str)

    def on_emoji_clicked(self):
        date_key = self.current_date.strftime("%Y-%m-%d")
        day_meal_data = self.meal_data.get(date_key, {})
    
        self.meal_data_changed.emit(date_key, day_meal_data)  
        self.jump_to_week_day.emit(date_key)
    ...
    def update_mood(self):
        ...
        all_foods = []
        for day in self.meal_data.values():
            for meal_list in day.values():
        ...
        self.meal_data_changed.emit("week_update", self.meal_data)
